    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `notes` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `title` TEXT NOT NULL, `content` TEXT NOT NULL, `categoryId` INTEGER, FOREIGN KEY (`categoryId`) REFERENCES `categories` (`id`) ON UPDATE NO ACTION ON DELETE NO ACTION)');
        await database.execute(
...
class _$NoteDao extends NoteDao {
  ...
  Future<List<Note>> searchNotes(String query) async {
    return _queryAdapter.queryList(
        'SELECT * FROM notes WHERE title LIKE ?1 OR content LIKE ?1 ORDER BY id DESC',
        mapper: (Map<String, Object?> row) => Note(id: row['id'] as int?, title: row['title'] as String, content: row['content'] as String, categoryId: row['categoryId'] as int?),
        arguments: [query]);
  }

  @override
  Future<List<Note>> getNotesWithCategories() async {
    return _queryAdapter.queryList(
        'SELECT notes.* FROM notes INNER JOIN categories ON notes.categoryId = categories.id ORDER BY notes.id DESC',
        mapper: (Map<String, Object?> row) => Note(
            id: row['id'] as int?,
            title: row['title'] as String,
            content: row['content'] as String,
